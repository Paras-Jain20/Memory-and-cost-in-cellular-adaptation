function centered_moments_adapt_time = mean_adaptation_time_periodic_env(N,T,q01,q10)
...
for k = 1:total_time_steps
    ...
    elseif(k > ceil(T/2) && k <= T)
        prob_adapt_time_01(k) = sum(v_0 * P1b_01^(ceil(T/2)) * P0b_01^(k-ceil(T/2)-1) * P0b_bc_01);
    elseif(k >= m*T && k <= m*T + ceil(T/2) )
        prob_adapt_time_01(k) = sum(v_0 * P1b_01^(ceil(T/2)) * P0b_01^(floor(T/2)) *(P1b_01^(ceil(T/2)) * P0b_01^(floor(T/2)))^(m-1) * P1b_01^(k-m*T-1) * P1b_bc_01);
    elseif(k > m*T + ceil(T/2)  && k <= (m+1)*T)
        prob_adapt_time_01(k) = sum(v_0 * P1b_01^(ceil(T/2)) * P0b_01^(floor(T/2)) *(P1b_01^(ceil(T/2)) * P0b_01^(floor(T/2)))^(m-1) * P1b_01^(ceil(T/2)) * P0b_01^(k-(m*T+ceil(T/2))-1) * P0b_bc_01);
    end
    if(mod(k,T) == 0)
        m = m+1;
    end
...
for k = 1:total_time_steps
    ...
    elseif(k > floor(T/2) && k <= T)
        prob_adapt_time_10(k) = sum(v_1 * P0bc_10^(floor(T/2)) * P1bc_10^(k-floor(T/2)-1) * P1bc_b_10);
    elseif(k > m*T && k <= m*T + floor(T/2) )
        prob_adapt_time_10(k) = sum(v_1 * P0bc_10^(floor(T/2)) * P1bc_10^(ceil(T/2)) *(P0bc_10^(floor(T/2)) * P1bc_10^(ceil(T/2)))^(m-1) * P0bc_10^(k-m*T-1) * P0bc_b_10);
    elseif(k > m*T + floor(T/2)  && k <= (m+1)*T)
        prob_adapt_time_10(k) = sum(v_1 * P0bc_10^(floor(T/2)) * P1bc_10^(ceil(T/2)) *(P0bc_10^(floor(T/2)) * P1bc_10^(ceil(T/2)))^(m-1) * P0bc_10^(floor(T/2)) * P1bc_10^(k-(m*T+floor(T/2))-1) * P1bc_b_10);
    end
    if(mod(k,T) == 0)
        m = m+1;
    end
