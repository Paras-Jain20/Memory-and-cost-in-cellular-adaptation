function centered_moments_adapt_time = mean_adaptation_time_periodic_env(N,T,q01,q10,Eh_bias)
...
k = 1;
    while(true)
        ...
        if(k+1> T)
            if(sum(abs(x_0(k+1,:)-x_0(k+1-T,:)))< 10^-4 && rem(k+1,T) == 0)
...
v_0 = x_0_ss(T_El,:)/sum(x_0_ss(T_El,:));
% v_0_temp = prob_analy(T_El,1:floor(q01*N)+1)/(sum(prob_analy(T_El,1:floor(q01*N)+1)));
...
for k = 1:total_time_steps
    ...
    elseif(k > T_Eh && k <= T)
        prob_adapt_time_01(k) = sum(v_0 * P1b_01^(T_Eh) * P0b_01^(k-T_Eh-1) * P0b_bc_01);
    elseif(k >= m*T && k <= m*T + T_Eh )
        prob_adapt_time_01(k) = sum(v_0 * P1b_01^(T_Eh) * P0b_01^(T_El) *(P1b_01^(T_Eh) * P0b_01^(T_El))^(m-1) * P1b_01^(k-m*T-1) * P1b_bc_01);
    elseif(k > m*T + T_Eh  && k <= (m+1)*T)
        prob_adapt_time_01(k) = sum(v_0 * P1b_01^(T_Eh) * P0b_01^(T_El) *(P1b_01^(T_Eh) * P0b_01^(T_El))^(m-1) * P1b_01^(T_Eh) * P0b_01^(k-(m*T+T_Eh)-1) * P0b_bc_01);
    end
    if(mod(k,T) == 0)
        m = m+1;
    end
...
for k = 1:total_time_steps
    ...
    elseif(k > T_El && k <= T)
        prob_adapt_time_10(k) = sum(v_1 * P0bc_10^(T_El) * P1bc_10^(k-T_El-1) * P1bc_b_10);
    elseif(k > m*T && k <= m*T + T_El )
        prob_adapt_time_10(k) = sum(v_1 * P0bc_10^(T_El) * P1bc_10^(T_Eh) *(P0bc_10^(T_El) * P1bc_10^(T_Eh))^(m-1) * P0bc_10^(k-m*T-1) * P0bc_b_10);
    elseif(k > m*T + T_El  && k <= (m+1)*T)
        prob_adapt_time_10(k) = sum(v_1 * P0bc_10^(T_El) * P1bc_10^(T_Eh) *(P0bc_10^(T_El) * P1bc_10^(T_Eh))^(m-1) * P0bc_10^(T_El) * P1bc_10^(k-(m*T+T_El)-1) * P1bc_b_10);
    end
    if(mod(k,T) == 0)
        m = m+1;
    end
