function [prob_state_1,prob_state_1_given_1] = undated_memory_cell_state_and_cond_prob_cell_state_periodic_env(N,T,q01,q10)
...
k = 1;
    while(true)
        ...
        if(k+1> T)
            if(sum(abs(x_0(k+1,:)-x_0(k+1-T,:)))< 10^-6 && sum(abs(x_1(k+1,:)-x_1(k+1-T,:)))< 10^-6 && rem(k+1,T) == 0)
